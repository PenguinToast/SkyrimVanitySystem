...
void KeyHandler::RegisterSink() {
  ...
  if (inputMgr) {
    ...
    logger::critical(
        "Failed to get InputDeviceManager. KeyHandler sink NOT registered!");
  }
}

[[nodiscard]] KeyHandlerEvent KeyHandler::Register(uint32_t dxScanCode,
                                                   KeyEventType eventType,
                                                   KeyCallback callback) {
  if (!callback) {
    logger::warn("Attempted to register a null callback for key 0x{:X}",
                 dxScanCode);
  ...
  logger::info(
      "Registering callback with handle {} for key 0x{:X}, event type {}",
      handle, dxScanCode,
      (eventType == KeyEventType::KEY_DOWN ? "DOWN" : "UP"));

  auto &keyCallbacks = _registeredCallbacks[dxScanCode];
  auto &targetMap = (eventType == KeyEventType::KEY_DOWN) ? keyCallbacks.down
                                                          : keyCallbacks.up;
...
void KeyHandler::Unregister(KeyHandlerEvent handle) {
  ...
  if (keyCallbacksIt != _registeredCallbacks.end()) {
    ...
    auto &targetMap = (info.type == KeyEventType::KEY_DOWN) ? keyCallbacks.down
                                                            : keyCallbacks.up;
    ...
    if (removedCount > 0) {
      logger::info(
          "Unregistered callback with handle {} for key 0x{:X}, event type {}",
          handle, info.key,
          (info.type == KeyEventType::KEY_DOWN ? "DOWN" : "UP"));
    } else {
      logger::error("Inconsistency detected: Handle {} found in handle map but "
                    "corresponding callback not found for key 0x{:X}.",
                    handle, info.key);
    }

    if (keyCallbacks.down.empty() && keyCallbacks.up.empty()) {
      logger::debug("Removing empty key entry 0x{:X} from callback map.",
                    info.key);
    ...
    logger::error("Inconsistency detected: Handle {} found in handle map but "
                  "key 0x{:X} not found in callback map.",
                  handle, info.key);
  }
}
...
RE::BSEventNotifyControl KeyHandler::ProcessEvent(
  ...
  for (auto event = *a_eventList; event; event = event->next) {
    ...
    if (keyCallbacksIt != _registeredCallbacks.end()) {
      ...
      const auto &targetMap = (eventType == KeyEventType::KEY_DOWN)
                                  ? keyCallbacks.down
                                  : keyCallbacks.up;
